...
try:
    from config import SAVE_DIR as _SAVE_DIR_CFG
except Exception:
    _SAVE_DIR_CFG = "captures"
...
class SplashScreen:
    def __init__(self, root):
        ...
        tk.Label(inner, text="🔱", font=("Segoe UI", 36),
                 bg=CARD, fg=BLUE).pack(pady=(30, 4))

        tk.Label(inner, text="Trident Drone Mapping System 🔱",
                 font=("Segoe UI", 18, "bold"),
                 bg=CARD, fg=BLUE).pack()

        tk.Label(inner, text="Aerial image capture & geotagging",
                 font=("Segoe UI", 10),
                 bg=CARD, fg=MUTED).pack(pady=(2, 20))

        # Progress bar track
        bar_track = tk.Frame(inner, bg="#13131f",
                             height=4, width=360)
        bar_track.pack()
        ...
        # Version / credit line
        tk.Label(inner, text="v1.0  ·  Powered by MAVLink & OpenCV",
                 font=("Segoe UI", 8),
                 bg=CARD, fg="#44475a").pack(side="bottom", pady=12)
        ...
        try:
            ...
            if alpha > 0:
                self.splash.after(20,
                    lambda: self._fade(round(alpha - 0.05, 2)))
            else:
                self.splash.destroy()
...
class MappingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Trident Drone Mapping System 🔱")
        self.root.geometry("720x900")
        self.root.minsize(540, 500)
        self.root.resizable(True, True)
        self.root.configure(bg="#eaff2a")
    ...
    def _build_ui(self):
        ...
        tk.Label(title_frame, text="Trident Drone Mapping System 🔱",
                 font=("Segoe UI", 16, "bold"), bg=BG, fg="#dbc21e").pack()
        tk.Label(title_frame, text="Automated aerial image capture",
                 font=FONT, bg=BG, fg=MUTED).pack(pady=(2, 0))
        ...
        def make_card(parent, col, label):
            card = tk.Frame(parent, bg=CARD, padx=10, pady=10,
        ...
        # --------------------------------------------------
        tk.Label(W, text="AIRCRAFT TELEMETRY",
                 font=("Segoe UI", 8), bg=BG, fg=MUTED).grid(
                     row=2, column=0, sticky="w", padx=PAD+4, pady=(6, 1))

        telem_outer = tk.Frame(W, bg=CARD, padx=14, pady=10,
                               highlightthickness=1, highlightbackground="#44475a")
        ...
        def telem_row(parent, row, col_offset, label):
            tk.Label(parent, text=label, font=FONT, bg=CARD,
                     fg=MUTED, width=16, anchor="w").grid(
                         row=row, column=col_offset,
                         sticky="w", pady=2, padx=(0, 6))
            var = tk.StringVar(value="—")
            tk.Label(parent, textvariable=var, font=FONT_BOLD,
                     bg=CARD, fg=FG, anchor="w", width=16).grid(
                         row=row, column=col_offset+1, sticky="w")
            return var
        ...
        # --------------------------------------------------
        tk.Label(W, text="CAMERA MOUNT  (Telem 2 / MOUNT_STATUS)",
                 font=("Segoe UI", 8), bg=BG, fg=MUTED).grid(
                     row=4, column=0, sticky="w", padx=PAD+4, pady=(6, 1))

        mount_outer = tk.Frame(W, bg=CARD, padx=14, pady=10,
                               highlightthickness=1, highlightbackground="#44475a")
        ...
        def mount_row(parent, row, col_offset, label):
            tk.Label(parent, text=label, font=FONT, bg=CARD,
                     fg=MUTED, width=16, anchor="w").grid(
                         ...
                         sticky="w", pady=2, padx=(0, 6))
            var = tk.StringVar(value="—")
            lbl = tk.Label(parent, textvariable=var, font=FONT_BOLD,
                           bg=CARD, fg=FG, anchor="w", width=16)
        ...
        self.var_img_dir,  self.lbl_img_dir  = mount_row(mount_grid, 1, 2, "Img Direction")

        # --------------------------------------------------
        # ROW 6 — Image counter
        # --------------------------------------------------
        count_frame = tk.Frame(W, bg=CARD, padx=14, pady=12,
                               highlightthickness=1, highlightbackground="#44475a")
        ...
        tk.Label(count_inner, textvariable=self.var_count,
                 font=("Segoe UI", 44, "bold"), bg=CARD, fg=GREEN).pack(side="left")
        tk.Label(count_inner, text=" images captured",
                 font=("Segoe UI", 13), bg=CARD, fg=FG).pack(
                     side="left", anchor="s", pady=12)

        # --------------------------------------------------
        # ROW 6b — Mission complete banner (hidden until fired)
        # --------------------------------------------------
        self.mission_banner = tk.Frame(W, bg="#1a3a1a", padx=14, pady=12,
                                       highlightthickness=2,
                                       highlightbackground="#50fa7b")
        # Not gridded yet — shown only on mission complete

        self.var_banner = tk.StringVar(value="")
        tk.Label(self.mission_banner, textvariable=self.var_banner,
                 font=("Segoe UI", 13, "bold"),
                 bg="#1a3a1a", fg="#50fa7b",
                 justify="center").pack()

        tk.Button(self.mission_banner,
                  ...
                  padx=12, pady=6,
                  command=self._reset_mission).pack(pady=(8, 0))
        ...
        tk.Label(btn_frame,
                 ...
                 font=("Segoe UI", 8), bg=BG, fg=MUTED).pack(pady=(3, 0))
        ...
        tk.Label(W,
                 ...
                 font=("Segoe UI", 8),
                 bg="#13131f", fg=MUTED,
                 anchor="w", padx=8).grid(
                     row=9, column=0, sticky="ew", pady=(4, 8))
    ...
    def _init_system(self):
        ...
        # --- MAVLink ---
        sp("Connecting to MAVLink (waiting for heartbeat)...", 45)
        self._update_status("Connecting to MAVLink  udp:127.0.0.1:14551 ...")
        ...
        try:
            ...
            self.tcp_listener = TCPListener(
                host="0.0.0.0", port=5555, callback=on_tcp)
        ...
        while self.running:
            if self.mav_reader:
                ...
                def update(t=t):
                    ...
                    fix = t["fix_type"]
                    if fix >= 3:
                        self._set_card(self.var_gps, self.lbl_gps,
                                       f"3D ({fix})", "#50fa7b")
                    elif fix == 2:
                        self._set_card(self.var_gps, self.lbl_gps,
                                       "2D Fix", "#f1fa8c")
                    ...
                    self.var_rel_alt.set(
                        ...
                        if t['relative_alt'] is not None else "—")
                    ...
                    if cam_pan is not None:
                        ...
                        if cam_tilt is not None and abs(cam_tilt + 90) < 5:
                            self._set_card(self.var_nadir, self.lbl_nadir,
                                           "Confirmed ✓", "#50fa7b")
                        else:
                            self._set_card(self.var_nadir, self.lbl_nadir,
        ...
        try:
            ...
            if (not telemetry
                ...
                self._log(
                    ...
                    f"sats={telemetry.get('satellites', 0)})", "WARN")
                return
            ...
            if cam_pan is not None:
                ...
                dir_str = (f"Yaw fallback: "
                           f"{math.degrees(telemetry['yaw']) % 360:.1f}°")
            else:
                dir_str = "Dir: N/A"
            ...
            self._log(
                ...
                f"{telemetry['alt']:.1f}m) | {dir_str}", "OK")
